...
//   const isLoading = useSelector((state: any) => state.loading.isLoading); // Access global loading state
...
//         await toggleApprovalStatus(user._id, action);
//         Swal.fire({
//           title: "Success!",
...
export default function ProfileHeader({ user, config, setUser }: ProfileHeaderProps) {
  ...
  const handleAdminAction = async (action: 'approved' | 'rejected') => {
    ...
    // Prompt for rejection message if rejecting
    if (action === "rejected") {
      const result = await Swal.fire({
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8">
            ...
            <div className="text-center lg:text-left flex-1">
              ...
              <div className="flex flex-wrap gap-3 justify-center lg:justify-start">
                ...
                {user.approvedByAdmin === "rejected" && user.rejectionMessage && (
                  <span className="bg-red-100 text-red-800 px-4 py-2 rounded-full text-sm font-semibold shadow-sm">
