...
import { getTeacherProfile, addTeacherCourse } from "../../API/teacherApi";
...
export function CreateCourseForm() {
  ...
  useEffect(() => {
    const validateTeacherProfile = async () => {
      try {
        ...
        if (profile.approvedByAdmin !== 'approved') {
          throw new Error('Teacher profile must be approved to create courses');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.chapters.forEach((chapter, index) => {
        ...
        if (chapter.video) {
          submitData.append('chapters', chapter.video); // Matches backend expectation
        }
      });

      await addTeacherCourse(submitData);
      dispatch(showNotification({
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Course Creation Studio</h1>
          <h2 className="text-xl text-blue-600 font-medium">Design Your Perfect Learning Experience</h2>
        </div>
